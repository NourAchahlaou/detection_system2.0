...
const DetectionLotForm = ({
  ...
  onClose,
  onSubmit,
  onCreateLot, // New prop for creating lot only
  onCreateLotAndDetect,
  cameraId,
  targetLabel,
  detectionOptions,
  isSubmitting = false,
  ...
  // Handle Create Lot Only - NEW
  const handleCreateLotOnly = () => {
    ...
    console.log('Creating lot with data:', submissionData);
  ...
  // Handle form submission (existing lot or create and detect)
  const handleSubmit = () => {
    if (!isValid || isSubmitting) return;
    ...
    if (formData.useExistingLot) {
      ...
      // Create new lot and detect
      onCreateLotAndDetect({
        type: 'create_and_detect',
        lotName: formData.lotName.trim(),
        expectedPieceId: parseInt(formData.expectedPieceId),
        expectedPieceNumber: parseInt(formData.expectedPieceNumber),
        expectedPieceLabel: formData.expectedPieceLabel,
  ...
  const formContent = (
    <Stack spacing={2}>
      ...
      {/* Action Buttons - UPDATED WITH CREATE ONLY OPTION */}
      <Stack direction={isInSidebar ? "column" : "row"} spacing={1} sx={{ justifyContent: 'flex-end' }}>
        ...
        {/* Create Lot Only Button - NEW */}
        {!formData.useExistingLot && onCreateLot && (
          <Button
            variant="outlined"
            size="small"
            onClick={handleCreateLotOnly}
            ...
            sx={{ fontSize: '0.8rem' }}
          >
            {isSubmitting ? 'Creating...' : 'Create Lot Only'}
          </Button>
        )}
        ...
        <Button
          ...
          startIcon={
            isSubmitting ? (
              ...
              formData.useExistingLot ? <PlayArrow fontSize="small" /> : <Add fontSize="small" />
            )
          }
          sx={{ fontSize: '0.8rem' }}
        >
          {isSubmitting ? 'Processing...' : (
            formData.useExistingLot ? 'Start Detection' : 'Create & Detect'
          )}
        </Button>
      </Stack>
      ...
      <Alert severity="info" sx={{ mt: 2, fontSize: '0.8rem' }}>
        ...
        <Typography variant="body2" component="div" sx={{ fontSize: '0.75rem' }}>
          {formData.useExistingLot ? (
            <>
              1. Detection on camera {cameraId}<br/>
              2. Results tracked in selected lot<br/>
              3. New detection session created<br/>
              4. Lot status updated if target found
            </>
          ) : (
            <>
              1. New lot "{formData.lotName || '[Name]'}" created<br/>
              2. Expected piece: {formData.expectedPieceLabel || '[Select piece]'} (ID: {formData.expectedPieceId || '[Auto-filled]'})<br/>
              3. {onCreateLot ? 'Option to create lot only or create & detect' : 'Detection starts automatically'}<br/>
              4. Stream frozen with results
            </>
          )}
        </Typography>
      </Alert>
