...
const IdentificationInfoPanel = ({
  ...
  // System mode props - Fixed prop names
  identificationState,  // Changed from detectionState
  systemProfile,
  ...
  // Utils - Fixed prop name
  IdentificationStates  // Changed from DetectionStates
}) => {
...
// Fixed toggle functions - replace your existing toggle functions with these:
  ...
  const getModeInfo = () => {
    if (currentStreamingType === 'basic') {
      return {
        icon: <Smartphone />,
        title: 'BASIC MODE',
        description: 'Lot-Based Identification - Tracked identification with database integration'
      };
    }
    return {
      icon: <Computer />,
      title: 'OPTIMIZED MODE',
      description: 'High-Performance Identification - Advanced streaming with optimizations'
    };
  };
  ...
  const hasSystemMode = identificationState === IdentificationStates?.READY && systemProfile;
                ...
                <Box sx={{ display: 'flex', alignItems: 'center' }}>
                  ...
                  <Tooltip title="Run Performance Test" arrow>
                    <IconButton 
                      ...
                      disabled={identificationState !== identificationState.READY}
                      sx={{ 
                        color: '#1976d2',
                        '&:hover': {
                          backgroundColor: alpha('#1976d2', 0.08)
...
export default IdentificationInfoPanel;