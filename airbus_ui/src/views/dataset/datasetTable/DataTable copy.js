...
export default function DataTable({ 
  ...
  const handleTrainGroup = useCallback(async (event, group) => {
    console.log('=== handleTrainGroup START ===');
    console.log('Event:', event);
    console.log('Group:', group);
    
    // CRITICAL: Prevent event propagation to avoid accordion interference
    if (event) {
      event.preventDefault();
      event.stopPropagation();
    }
    ...
    if (!onBatchTrain) {
      console.error('❌ onBatchTrain is null or undefined');
      alert('Training function not available');
      return;
    }

    if (typeof onBatchTrain !== 'function') {
      console.error('❌ onBatchTrain is not a function, it is:', typeof onBatchTrain);
      alert('Invalid training function');
      return;
    }

    if (!group || !group.pieces || !Array.isArray(group.pieces)) {
      console.error('❌ Invalid group data:', group);
      alert('Invalid group data');
      return;
    }

    // Get pieces that need training (annotated but not trained)
    const piecesToTrain = group.pieces.filter(piece => 
      piece.is_annotated && !piece.is_yolo_trained
    );
    
    console.log('Pieces in group:', group.pieces.length);
    console.log('Annotated pieces:', group.pieces.filter(p => p.is_annotated).length);
    console.log('Already trained pieces:', group.pieces.filter(p => p.is_yolo_trained).length);
    console.log('Pieces to train:', piecesToTrain.length);
    
    if (piecesToTrain.length === 0) {
      console.log('⚠️ No pieces available for training in this group');
      alert(`No pieces in group "${group.name}" are ready for training.\n\nRequirements:\n- Pieces must be annotated\n- Pieces must not already be trained`);
      return;
    }
    ...
    if (trainingInProgress) {
      ...
      alert('Another training session is already in progress. Please wait for it to complete.');
      return;
    }

    try {
      const pieceLabels = piecesToTrain.map(piece => piece.label);
      console.log('🚀 Starting training for labels:', pieceLabels);
      
      // Show immediate feedback
      console.log('Calling onBatchTrain...');
      
      // FIXED: Now properly await the result
      const result = await onBatchTrain(pieceLabels);
      console.log('Training function result:', result);
      
      // Check if training was successful
      if (result && result.success) {
        console.log('✅ Training initiated successfully');
      } else {
        console.warn('⚠️ Training may have failed:', result);
      ...
      console.error('❌ Error in handleTrainGroup:', error);
      alert(`Failed to start training: ${error.message || error}`);
    }
    
    console.log('=== handleTrainGroup END ===');
