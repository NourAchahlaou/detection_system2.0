...
async def get_user_profile_tab_info(user_id: int, session: Session) -> ProfileTabResponse:
    ...
    access_level ="Level 1" if user.role == RoleType.ADMIN else "Level 2" if user.role == RoleType.DATA_MANAGER and user.role == RoleType.OPERATOR else "Level 3"
    access_description ="Admin" if user.role == RoleType.ADMIN else "Standard data manager" if user.role == RoleType.DATA_MANAGER else  "Standard operator" if user.role == RoleType.OPERATOR else "Auditor"
