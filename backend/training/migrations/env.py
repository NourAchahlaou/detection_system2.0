...
print(f"training service DATABASE_URI: {settings.DATABASE_URI}")
...
OWNED_TABLES = {
    'training_session',  # This service owns training s
}
...
def create_cross_schema_foreign_keys(connection):
    ...
    try:
        ...
        result = connection.execute(text("""
            ...
            WHERE constraint_name = 'fk_training_session_piece_id'
            AND table_schema = 'training'
        """))
        
        if result.scalar() == 0:
            # Create FK constraint for training_session.piece_id -> artifact_keeper.piece.id
            connection.execute(text("""
                ALTER TABLE training.training_session
                ADD CONSTRAINT fk_training_session_piece_id
                FOREIGN KEY (piece_id)
                REFERENCES artifact_keeper.piece(id)
                ON DELETE CASCADE
            """))
            print("✓ Created FK constraint: training_session.piece_id -> artifact_keeper.piece.id")
        else:
            print("✓ FK constraint already exists: training_session.piece_id -> artifact_keeper.piece.id")
        ...
        print("Run artifact_keeper migrations first, then re-run training migrations.")
