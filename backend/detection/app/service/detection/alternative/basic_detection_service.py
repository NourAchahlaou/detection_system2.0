...
class BasicDetectionProcessor:
    ...
    def get_lot_sessions(self, lot_id: int, db: Session) -> List[Dict[str, Any]]:
        """Get all detection sessions for a lot with detected pieces data - UPDATED"""
        try:
            ...
            # Get the lot info to determine the expected piece label
            lot_info = self.get_detection_lot(lot_id, db)
            if not lot_info:
                logger.error(f"Lot {lot_id} not found when getting sessions")
                return []
            
            # Get the expected piece label from the database using expected_piece_id
            target_piece_label = "target"  # Default fallback
            if lot_info.expected_piece_id:
                try:
                    from detection.app.db.models.piece import Piece  # Adjust import path as needed
                    piece = db.query(Piece).filter(Piece.id == lot_info.expected_piece_id).first()
                    if piece and hasattr(piece, 'piece_label'):
                        target_piece_label = piece.piece_label
                        logger.info(f"Found target piece label '{target_piece_label}' for lot {lot_id}")
                    else:
                        logger.warning(f"Piece with ID {lot_info.expected_piece_id} not found or has no piece_label")
                except Exception as e:
                    logger.error(f"Error getting piece label for piece_id {lot_info.expected_piece_id}: {e}")
            ...
            for session_number, session in enumerate(sessions, 1):
                ...
                # Use the actual target piece label instead of hardcoded "target"
                target_piece = target_piece_label
                
                # Determine detected piece for display (use the most confident correct detection or first detection)
                detected_piece = "unknown"
                if detected_pieces:
                    correct_pieces = [p for p in detected_pieces if p['is_correct_piece']]
                    if correct_pieces:
                        # Use the most confident correct piece
                        best_correct = max(correct_pieces, key=lambda p: p['confidence_score'])
                        detected_piece = best_correct['detected_label']
                    else:
                        # Use the most confident detection overall
                        best_detection = max(detected_pieces, key=lambda p: p['confidence_score'])
                        detected_piece = best_detection['detected_label']
                
                # Build session data matching what your frontend expects
                session_data = {
                    # Core session data
                    'id': session.id,
                    ...
                    'confidence': session.confidence_score,  # Alias for frontend compatibility
                    'is_target_match': session.is_target_match,
                    'isTargetMatch': session.is_target_match,  # Alias for frontend compatibility
                    'detection_rate': session.detection_rate,
                    
                    # Target and detected pieces for display
                    'targetPiece': target_piece,  # Now uses actual piece label
                    ...
                    'processingTime': getattr(session, 'processing_time', 1000),  # Default if not stored
                    
                    # Detected pieces data
                    'detected_pieces': detected_pieces,
                    'detectedPieces': detected_pieces,  # Alias for frontend compatibility
                    'detected_pieces_count': len(detected_pieces)
                }
                sessions_data.append(session_data)
            
            logger.info(f"Retrieved {len(sessions_data)} sessions for lot {lot_id} with target piece '{target_piece_label}'")
