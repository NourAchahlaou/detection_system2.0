...
export PGPASSWORD="$DB_PASSWORD"
psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -c "CREATE SCHEMA IF NOT EXISTS detection;"
...
VERIFICATION_RESULT=$(psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -t -c "
    ...
    AND table_name IN ('piece', 'piece_image', 'camera', 'cameraSettings');
")
...
if [ "$ACTUAL_TABLES" -eq "$EXPECTED_TABLES" ]; then
    ...
    echo "Available tables in detection schema:"
...
exec uvicorn detection.app.main:app --host 0.0.0.0 --port 8000 --reload